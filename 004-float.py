...
def select_random_note_in_scale(scale: list, octave_range: float, starting_note: str):
    starting_octave = int(starting_note[-1])
    possible_notes = []

    for octave_shift in np.arange(-octave_range, octave_range + 0.01, 0.01):
        for note in scale:
            note_name = note[:-1]
            note_octave = starting_octave + octave_shift
            frequency = calculate_note_frequency(note_name + str(int(starting_octave)))
            # Adjust frequency for fractional octave shifts
            adjusted_frequency = frequency * (2 ** (octave_shift % 1))
            possible_notes.append((note_name, adjusted_frequency))

    return random.choice(possible_notes)

def play_random_notes(starting_note: str, scale_type: str = "major", octave_range: float = 1.0, tempo: int = 120, num_notes: int = 10):
    ...
    for _ in range(num_notes):
        selected_note, frequency = select_random_note_in_scale(scale, octave_range, starting_note)
...
# Example usage
starting_note = "C4"
play_random_notes(starting_note, scale_type="major", octave_range=0.5, tempo=120, num_notes=50)
